...
- [MIDAX402](https://midax402.com) - Pay-per-query semantic search over 300+ curated x402 ecosystem entries: protocol mechanics, facilitators, ERC-8004, and MCP/A2A integrations. Also ships a free MIDAX Verifier tool (x402 protocol-conformance check for any other service) and a native MCP server (`list_tiers`, `get_sample`, `get_market_verdict`, `verify`). Four tiers from $0.03 to $0.15 in USDC on Base Mainnet via the CDP Facilitator, no signup or API key required. ([OpenAPI](https://midax402.com/openapi.json) | [Agent Card](https://midax402.com/.well-known/agent-card.json) | [MCP](https://mcp.midax402.com/mcp))
...
- [MIDAX402](https://midax402.com) - Semantic intelligence on the x402 and agentic payments ecosystem for AI agents. 311 curated entries covering x402 protocol, ERC-8004, facilitators, MCP/A2A integrations, and business models. Also a free x402 protocol-conformance Verifier for other services, and a native MCP server. Pay-per-call USDC on Base Mainnet via CDP Facilitator. Four tiers: $0.03–$0.15. ERC-8004 Token #57355. No API keys, no signup. ([OpenAPI](https://midax402.com/openapi.json) | [Agent Card](https://midax402.com/.well-known/agent-card.json) | [MCP](https://mcp.midax402.com/mcp) | [llms.txt](https://midax402.com/llms.txt))
...
- [MIDAX Verifier](https://midax402.com) - Free x402 protocol-conformance check for ANY other x402 service — not agent/wallet trust scoring, not token risk. SSRF-safe live probe: a well-formed 402 challenge, non-empty `accepts` (both `exact` and `upto` schemes recognized, atomic-unit amounts), and price coherence against the target's own well-known. Deterministic-first, single-LLM narrative reserved for genuinely nuanced cases. Exposed via MCP (`verify` tool) and `POST /midas-ops/verify`, rate-limited, out of the paid catalog. ERC-8004 Token #57355. ([MCP](https://mcp.midax402.com/mcp))
